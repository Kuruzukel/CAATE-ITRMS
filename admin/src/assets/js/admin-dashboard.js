...
function updateCourseEnrollmentUI(data) {
    ...
    // Update the donut chart with enrollment data
    updateCourseEnrollmentChart(data);
}

// Function to update the course enrollment donut chart
function updateCourseEnrollmentChart(data) {
    if (typeof ApexCharts === 'undefined') {
        setTimeout(() => updateCourseEnrollmentChart(data), 500);
        return;
    }
    ...
    // Prepare chart data
    const series = data.topCourses.map(course => course.enrollmentCount);
    const labels = data.topCourses.map(course => {
    ...
    const colors = ['#696cff', '#03c3ec', '#71dd37', '#ffab00', '#ff3e1d'];

    // Create or update chart
    if (!window.courseEnrollmentChartInstance) {
        const chartConfig = {
            chart: {
                height: 165,
                width: 130,
        ...
        window.courseEnrollmentChartInstance = new ApexCharts(chartElement, chartConfig);
        window.courseEnrollmentChartInstance.render();
    } else {
        // Update existing chart
        window.courseEnrollmentChartInstance.updateOptions({
            labels: labels,
            series: series,
            colors: colors
        });
