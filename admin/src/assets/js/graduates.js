document.addEventListener('DOMContentLoaded', function () {
    ...
    function applyGraduateFilters() {
        ...
        const filteredCards = allCards.filter((card) => {
            ...
            const matchesSearch =
                !searchTerm || name.includes(searchTerm) || traineeId.includes(searchTerm);
    ...
    function updateCertificationCard(selectedCert, count) {
            ...
            if (result.success && result.data && Array.isArray(result.data)) {
                ...
                const graduatesPreviousMonth = graduates.filter((grad) => {
                    if (grad.graduation_date) {
                        const gradDate = new Date(grad.graduation_date);
                        return (
                            gradDate.getFullYear() === previousMonthYear && gradDate.getMonth() === previousMonth
                        );
                    }
                    return false;
                }).length;
                const yearPercentChange =
                    graduatesPreviousYear > 0
                        ? Math.round(
                            ((graduatesThisYear - graduatesPreviousYear) / graduatesPreviousYear) * 100,
                        )
                        : 0;
                const monthPercentChange =
                    graduatesPreviousMonth > 0
                        ? Math.round(
                            ((graduatesThisMonth - graduatesPreviousMonth) / graduatesPreviousMonth) * 100,
                        )
                        : 0;
                ...
                if (yearPercent) {
                    ...
                    yearPercent.className = isPositive
                ...
                if (monthPercent) {
                    ...
                    monthPercent.className = isPositive
                        ? 'text-success fw-semibold'
                        : 'text-danger fw-semibold';
    ...
    function addDeleteButtonsToExistingCards() {
        const graduateCards = document.querySelectorAll(
            '.row.row-cols-1.row-cols-md-2.row-cols-lg-3.row-cols-xl-4 .col',
        );
        graduateCards.forEach((col) => {
            ...
            deleteBtn.className =
                'btn btn-danger position-absolute top-0 end-0 m-2 delete-graduate-btn rounded-circle p-0';
            ...
            deleteBtn.style.cssText =
                'width: 38px; height: 28px; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(0,0,0,0.3);';
        ...
        saveNewGraduateBtn.addEventListener('click', async function () {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    modal.hide();
                    document.getElementById('addGraduateForm').reset();
                    document.getElementById('addGraduateImagePreview').src =
    ...
    function exportGraduatesToCSV() {
        const csvData = [['Name', 'Student ID', 'Course', 'Certification', 'Graduation Date', 'Email']];
    ...
    document.getElementById('confirmDeleteGraduateBtn')?.addEventListener('click', async function () {
        ...
        try {
            const response = await fetch(
                `${config.api.baseUrl}/api/v1/graduates/${graduateToDelete.mongoId}`,
                {
                    method: 'DELETE',
                    headers: {
    ...
    document.getElementById('confirmExportBtn').addEventListener('click', function () {
        const selectedCourses = Array.from(
            document.querySelectorAll('.export-course-filter:checked'),
        ).map((cb) => cb.value);
        ...
        link.setAttribute(
            'download',
            'graduates_list_' + new Date().toISOString().split('T')[0] + '.csv',
        );
        link.style.visibility = 'hidden';
        document.body.appendChild(link);
        link.click();
