...
class NotificationManager {
    ...
    async loadNotifications() {
        try {
            const stored = localStorage.getItem('trainee_notifications');
            if (stored) {
    ...
    saveNotifications() {
        try {
            localStorage.setItem('trainee_notifications', JSON.stringify(this.notifications));
        } catch (error) {
    ...
    async notifyRegistrationStatus(data) {
        await this.addNotification({
            ...
            message: `Your registration is now ${data.status}`
        });
    }
    ...
    async notifyApplicationStatus(data) {
        await this.addNotification({
            ...
            message: `Your application is now ${data.status}`
        });
    }
    ...
    async notifyAdmissionStatus(data) {
        await this.addNotification({
            ...
            message: `Your admission is now ${data.status}`
        });
    }
    ...
    async notifyAppointmentStatus(data) {
        await this.addNotification({
            ...
            message: `Your appointment is now ${data.status}`
        });
    }
