...
// Competencies data to UPDATE existing courses
$competencies = [
...
try {
    ...
    // Update courses with competencies
    echo "Updating courses with competencies...\n\n";
    $updatedCount = 0;
    
    foreach ($competencies as $competency) {
        $result = $collection->updateOne(
            ['course_code' => $competency['course_code']],
            ['$set' => [
                'basic_competencies' => $competency['basic_competencies'],
                'common_competencies' => $competency['common_competencies'],
        ...
        if ($result->getModifiedCount() > 0) {
            ...
            echo "✓ Updated: {$competency['course_title']}\n";
            echo "  - Basic: {$basicCount} | Common: {$commonCount} | Core: {$coreCount} | Total: {$totalCount}\n\n";
        } else {
            echo "⚠ Course not found: {$competency['course_code']}\n\n";
        }
    }
    
    echo "========================================\n";
    echo "Update completed successfully!\n";
    echo "Total courses updated: {$updatedCount}\n";
    echo "========================================\n";
